...
def test_in_bounds() -> None:
    ...
    assert board.in_bounds(Position(3, 2))
    assert not board.in_bounds(Position(-1, 1))
    assert not board.in_bounds(Position(3.01, 1))
    assert not board.in_bounds(Position(1, 2.01))


def test_clamp() -> None:
    board = Board(width=4, height=5)
    assert board.clamp(Position(-2, 2)) == Position(0, 2)
    assert board.clamp(Position(10, -9)) == Position(4, 0)
